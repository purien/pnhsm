...
#define START_TIME 10000   // us TIME for I2C Ready 2500 pour decoder 3000 pour switch SDA low 1us a 1,8ms ou 2,4ms 
#define GUARD_TIME 250000  // us after reset minimum 200ms
#define MAX_RETRY_HW    2
#define MAX_RETRY_CRC   2
#define MAX_RETRY_I2CRX 2 // 1ms per retry

#define PDEBUG 21  // Debug PIN
...
cpulse()   ;
MUTEX_UNLOCK(Pmutex[M_SYSTEM+2]);
if (i2c_fdebug) gPrintf(SCR,"Failed to read from the i2c bus req %d read %d adr=%u\n",len,err,adr);
return -1;
}
...
if (*tt >= dt)
  { if (i2c_fdebug) gPrintf(SCR,"%s %u\n","I2C Rx Timeout adr=",i2c_adr);
    error_timeout++;
    return -1;
  }

err= rxi2c(bufi2c,nb,i2c_adr);
// nb= 1 return 0 or NAD
// nb= 1 return PCB
// nb= 1 return LEN
// nb= x<=32 return x bytes

while(err != nb) 
{ 
  lct++;
  if (lct > MAX_RETRY_I2CRX) break;
...
{ int err,i;
  ...
  if (file_i2c < 0)
  {  if (i2c_fdebug) gPrintf(SCR,"Error..can't open i2c-1 dev\n");
     return -1;
  }
  
  // ioctl(file_i2c,I2C_TIMEOUT,1);// 10ms 
 ...
 { if (i2c_fdebug)
   ...
   //0=E  1=LE 2=A0 3=A1 4=A2 5=A3
   if (fdecoder)
   { for(i=0;i<6;i++)
     { if (i2c_fdebug) gPrintf(SCR,"Pin %d => output\n",i2c_decoder_pins[i]);
       pinMode(i2c_decoder_pins[i], OUTPUT) ;
       if (i==1) digitalWrite (i2c_decoder_pins[i], HIGH)  ; // LE
       else      digitalWrite (i2c_decoder_pins[i], HIGH)  ; // Other
     }
   ...
   { for (i=0;i<1;i++) // for test
     I2C_Reset_Pin(i2c_reset_pins[i2c_adr[k]]); // RESET
   }
